#!/usr/bin/env python3
"""Orchestrator with Popen pip, subprocess model, real cancel, state save."""
...
class SetupController:
    ...
    def __init__(self, model_id="tiny", event_callback=None):
        ...
        assert SetupStage.READY not in self._stage_map
        self._load()
    ...
    def _run_process(self, cmd, timeout=300):
        try:
            ...
            try:
                _out, _err = self._active_process.communicate(timeout=timeout)
            except subprocess.TimeoutExpired:
                self._kill_active_process()
                return False
            if self._cancel_requested:
                return False
            return self._active_process.returncode == 0
        except OSError:
            return False
        finally:
            self._active_process = None
    ...
    def _step_download_model(self):
        ...
        return self._run_process(
            [VENV_PYTHON, rt, "download-model", self.sm.model_id], 600)
    ...
    def _step_verify(self):
        ...
        if not os.path.exists(rt) or not os.path.exists(VENV_PYTHON):
            return False
        return self._run_process(
            [VENV_PYTHON, rt, "verify-model", self.sm.model_id], 120)
        ...
        for i, stage in enumerate(order):
            ...
            if stage == SetupStage.CREATE_ENV and not os.path.exists(VENV_PYTHON):
                ...
                if os.path.exists(rt) and os.path.exists(VENV_PYTHON):
                    fail = not self._run_process(
                        [VENV_PYTHON, rt, "verify-model", self.sm.model_id], 30)
            if fail:
