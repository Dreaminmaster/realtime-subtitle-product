...
results = []
t.on_done(lambda ok,e,a: results.append((ok,e,a)))
thr = threading.Thread(target=t.start, daemon=True)
thr.start()
thr.join(5)
assert t.state == SUCCEEDED and len(results) == 1 and results[0][0]
...
thr2.start()
thr2.join(5)
assert t2.state == FAILED and len(r2) == 1 and not r2[0][0]
assert t2.attempt == 2
...
thr3.start()
thr3.join(5)
assert t3.state == FAILED and len(r3) == 1 and not r3[0][0]
...
r4 = []
t4.on_done(lambda ok,e,a: r4.append((ok,e,a)))
...
cnt = [0]
t5.on_done(lambda ok,e,a: cnt.__setitem__(0, cnt[0]+1))
